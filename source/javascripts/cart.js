...
updateCartCounts = (cart) => {
  ...
  const cartCountHiddenElements = document.querySelectorAll('.cart-num-items-hidden');
  ...
  cartCountHiddenElements.forEach((element) => {
    element.innerHTML = Format.pluralize(item_count, 'item', 'items');
  });
